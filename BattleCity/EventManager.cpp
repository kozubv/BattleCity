...
void EventManager::HandleEvent(sf::Event& l_event) {
  ...
  for (auto &b_itr : m_bindings){      
    ...
    for (auto &e_itr : bind->m_events){ 
                      // выполнение Ќ≈— ќЋ№ »’ условий
      EventType sfmlEvent = (EventType)l_event.type;
      if (e_itr.first != sfmlEvent) {  // если тип не совпадает с занесенным в список - пас
        continue; 
      }

      if (sfmlEvent == EventType::KeyDown || 
          sfmlEvent == EventType::KeyUp)  {
        //std::cout << " Press " << l_event.key.code << std::endl;
...
void EventManager::Update() {
  ...
  for (auto &b_itr : m_bindings){      
    Binding* bind = b_itr.second;    
    for (auto &e_itr : bind->m_events)  // дл¤ каждого
    {
      switch(e_itr.first)
      {
...
void EventManager::LoadBindings()
{
  ...
  bindings.open(R"(..\resources\keys.cfg)");
  //bindings.open(SharedContext::getFilePath("Keys"));
  if (!bindings.is_open()){
